...
public class MessageReceiver
{
    ...
    private boolean isStillReceiving = true;
    ...
    public String receiveMessage() throws ProtocolException
    {
        ...
        while(isStillReceiving)
        {
            ...
            //Check frame length is not bigger than MTU
            if(frame.length() > physicalLayer.getMTU())
            {
                throw new ProtocolException("Frame is greater than MTU size.");
            }
            ...
            if(segmentLength > frameSpaceLeft)
            {
                throw new ProtocolException("Message received is greater than amount of space left in frame.");
            }
            
            
              
            //Check segment length is not bigger than 99
            if(segmentLength > 99 )
            {
                throw new ProtocolException ("Segment Length Cannot be greater than 99.");
            }
            ...
            {
                throw new ProtocolException("Recieved message length differs from that calculated.");
            }
            ...
            if(checkSumFormatted.length()> 2 )
            {
                checkSumFormatted = Integer.toString(checkSum);
                checkSumFormatted = checkSumFormatted;
            }
            ...
            {
                throw new ProtocolException("Checksum calculated differs from that received.");
            }

            //build final string
            if(newMessageString == "")
            {
                newMessageString = oldMessage;
            }
            else
            {
